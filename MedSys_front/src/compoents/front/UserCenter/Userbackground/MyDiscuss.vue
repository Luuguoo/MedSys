  ...
  <div>
    <el-card class="box-card" shadow="always" style="border-radius: 20px;">
      <div slot="header" class="clearfix">
        <p align="left" style="font-size: 20px">我的评论</p>
      </div>
              ...
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <el-button type="primary" icon="el-icon-more-outline" size="mini" circle
                             @click="toDetail(scope.row.aid)"></el-button>
...
<script>
  export default {
    ...
    methods: {
      paging(curPage) {
        this.$axios.get('front/protects/discuss/list?curPage=' + curPage + '&pageSize=' + this.pageSize)
          .then(resp => {
            this.page = resp.data.data
          })
      },
      toDetail(aid) {
        this.$axios.get('front/protects/article/findByAid?aid=' + aid)
          .then(resp => {
      ...
      del(id) {
        this.$confirm('此操作将永久删除该文记录, 是否继续?', '提示', {
          ...
          this.$axios.get('front/protects/discuss/del?id=' + id)
            .then(resp => {
              if (resp.data.code === 1) {
                this.$message({
                  type: 'success',
