  ...
  <div>
    <el-card class="box-card" shadow="always" style="border-radius: 20px;">
      <div slot="header" class="clearfix">
        <p align="left" style="font-size: 20px">我的收藏</p>
      </div>
        ...
        <el-row>
          <el-col :span="24">
            <el-table :data="page.list" stripe style="width: 100%;">
              ...
              <el-table-column prop="addtime" label="收藏时间" width=""></el-table-column>
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <el-button type="primary" icon="el-icon-more-outline" size="mini" circle
                             @click="toDetail(scope.row.article)"></el-button>
...
<script>
  export default {
    ...
    methods: {
      paging(curPage) {
        this.$axios.get('front/protects/collect/list?curPage=' + curPage + '&pageSize=' + this.pageSize)
          .then(resp => {
      ...
      del(id) {
        this.$confirm('此操作将永久删除该文记录, 是否继续?', '提示', {
          ...
          this.$axios.get('front/protects/collect/del?id=' + id)
            .then(resp => {
              if (resp.data.code === 1) {
                this.$message({
                  type: 'success',
