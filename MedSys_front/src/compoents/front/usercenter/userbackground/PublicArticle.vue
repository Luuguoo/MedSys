  ...
  <div>
    <el-card class="box-card" shadow="always" style="border-radius: 20px;">
      <div slot="header" class="clearfix">
        <p align="left" style="font-size: 20px">添加文章</p>
      </div>
      <div style="min-height: 600px">
        <el-row>
          <el-col :span="20" :offset="4">
            <el-form style="width: 500px" ref="form" :model="article" :rules="rules" label-width="100px"
...
<script>
  export default {
    ...
    methods: {
      ...
      save() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            this.article.lname = sessionStorage.getItem("username")
            this.article.flag2 = 0
            ...
            this.$axios.post('front/protects/article/add', formData)
              .then(resp => {
                if (resp.data.code === 1) {
                  this.$message({
      ...
      getAtypeList() {
        var atypes = []
        this.$axios.get('front/protects/atype/findAll')
          .then(resp => {
            this.atypes = resp.data.data
            this.$axios.get('front/protects/users/getInfo')
              .then(resp => {
                var userLevel = resp.data.data.level
                if (userLevel === 1) {
                  return
                }
                this.atypes.forEach(item => {
                  if (item.isNeedVip === 0) {
                    atypes.push(item)
                  }
                })
