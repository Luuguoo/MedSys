  ...
  <div>
    <el-card class="box-card" shadow="always" style="border-radius: 20px;">
      <div slot="header" class="clearfix">
        <p align="left" style="font-size: 20px">审核文章</p>
      </div>
        ...
        <el-row>
          <el-col :span="24">
            ...
            <el-table :data="page.list" stripe style="width: 100%;">
              <el-table-column prop="atype.tname" label="用户名" width=""></el-table-column>
              ...
              <el-table-column width="400px">
                <template slot-scope="scope">
                  ...
                  <el-button type="warning" size="mini" round
                             @click="check(scope.row.aid,'审核通过')">审核通过
                  </el-button>
                  <el-button type="info" size="mini" round
                             @click="check(scope.row.aid,'审核不通过')">审核不通过
                  </el-button>
...
export default {
  ...
  methods: {
    paging(curPage) {
      this.$axios.get('back/article/list?curPage=' + curPage + '&flag=' + this.flag + '&tid=' + this.tid + '&lname=' + this.lname + '&title=' + this.title + '&pageSize=' + this.pageSize)
          .then(resp => {
    ...
    toDetail(article) {
      this.$router.push('/backHome/backarticleDetail?article=' + JSON.stringify(article) + '&page=check')
    },
    check(aid,checkResult){
      this.$axios.get('back/article/check?aid=' + aid+'&checkResult='+checkResult)
          .then(resp => {
            if (resp.data.code === 1) {
              this.$message({
                type: 'success',
    ...
    checkNoPass(aid){
      this.$axios.get('back/article/check?aid=' + aid+'&checkResult=审核通过')
          .then(resp => {
            if (resp.data.code === 1) {
              this.$message({
                type: 'success',
